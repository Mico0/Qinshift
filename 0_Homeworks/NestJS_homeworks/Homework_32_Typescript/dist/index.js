...
const studentArray = [student, student2, student3];
function calculateAverageGrade(students) {
  const studentArray = Array.isArray(students) ? students : [students];
  const allGrades = studentArray.flatMap((student) => student.grades);
  let sum = allGrades.reduce((acc, curr) => acc + curr, 0);
...
class CourseManager {
  ...
  getTopStudents(courseID, n) {
    ...
    let sorted = [...course.students].sort((a, b) => a.avgGrade - b.avgGrade);
    //! Way 2 to solve this

"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SkillsService = void 0;
const common_1 = require("@nestjs/common");
const promises_1 = require("node:fs/promises");
const node_path_1 = require("node:path");
const uuid_1 = require("uuid");
let SkillsService = class SkillsService {
    constructor() {
        console.log('skill service created');
    }
    SKILLS_PATH = (0, node_path_1.join)(process.cwd(), 'data', 'skills.json');
    async findAll() {
        const skillsJSON = await (0, promises_1.readFile)(this.SKILLS_PATH, 'utf-8');
        const skills = JSON.parse(skillsJSON);
        return skills;
    }
    async save(skills) {
        await (0, promises_1.writeFile)(this.SKILLS_PATH, JSON.stringify(skills, null, 2));
    }
    async findById(id) {
        const skills = await this.findAll();
        const foundSkill = skills.find((skill) => skill.id === id);
        if (!foundSkill)
            throw new common_1.NotFoundException('skill not found');
        return foundSkill;
    }
    async create(createData) {
        const skills = await this.findAll();
        const newSkill = {
            id: (0, uuid_1.v4)(),
            ...createData,
        };
        const updatedSkills = [...skills, newSkill];
        await this.save(updatedSkills);
        return newSkill;
    }
};
exports.SkillsService = SkillsService;
exports.SkillsService = SkillsService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [])
], SkillsService);
//# sourceMappingURL=skills.service.js.map